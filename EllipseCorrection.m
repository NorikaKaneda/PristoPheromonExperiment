...
% 別ファイルの関数を使用
ellipse_params = EstimationEllParam(Ellipse_Circumference); % 楕円の式のパラメーター
%重ねて表示
hold(TrajAxes, "on")
...
%軸の傾きは
EllipseData.theta = angle(EllipseData.F2(1)-EllipseData.F1(1)+EllipseData.F1(2)*1i-EllipseData.F2(2)*1i)
%長軸の長さは
...
hold(TrajAxes, "off")
saveas(gcf, append(Filename, "_PhotoEllipse.png"))
...
RotateMat = [cos(-EllipseData.theta),-sin(-EllipseData.theta);sin(-EllipseData.theta),cos(-EllipseData.theta)] % 回転行列
